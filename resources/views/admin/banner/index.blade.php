    ...
    <div class="page-content">
        <div class="at-pageform">
            <div class="form-group float-left w-100">
                <a href="{{route('create.banner')}}" style="background-color: #E2A750; padding: 10px 20px; border: 0; border-radius: 5px; line-height: 37px;">Add Banner</a>
            </div>
        <table class="table">
            <thead>
            <tr>
            ...
            @foreach($banners as $key => $banner)
                ...
                <td>{{$banner->text}}</td>
                <td>
                <form action="{{route('delete.banner',['id'=>$banner->id])}}" method="post">
                    @csrf
                    @method('DELETE')
